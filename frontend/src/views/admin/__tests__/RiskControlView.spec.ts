...
describe('admin RiskControlView', () => {
  ...
  it('preserves both engine drafts and saves thresholds independently', async () => {
    getConfig.mockResolvedValue({ ...baseConfig(), hit_action: 'route_account', route_account_id: 42 })
    const wrapper = mount(RiskControlView, { global: { stubs: { AppLayout: AppLayoutStub, BaseDialog: BaseDialogStub, Icon: true, Select: true, Toggle: true, Pagination: true, ModelWhitelistSelector: ModelWhitelistSelectorStub, ProxySelector: true } } })
    await flushPromises()
    await findButtonByText(wrapper, 'admin.riskControl.openSettings').trigger('click')
    ...
    expect(wrapper.text()).not.toContain('admin.riskControl.modelHint')
    ...
    expect(wrapper.text()).toContain('admin.riskControl.modelHint')
    await findButtonByText(wrapper, 'admin.riskControl.saveConfig').trigger('click')
    await flushPromises()
    expect(updateConfig).toHaveBeenCalledWith(expect.objectContaining({ engine: 'openai', hit_action: 'route_account', route_account_id: 42, engine_configs: {
