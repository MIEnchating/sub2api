    ...
    >
      ...
      >
        <div class="flex items-center justify-between gap-4">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.openai.modelNormalization') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.openai.modelNormalizationDesc') }}
            </p>
          </div>
          <Toggle
            v-model="openAIModelNormalizationEnabled"
            data-testid="create-openai-model-normalization-toggle"
            :aria-label="t('admin.accounts.openai.modelNormalization')"
          />
        </div>
      </div>
...
const buildOpenAIExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  if (accountCategory.value === 'oauth-based' && !openAIModelNormalizationEnabled.value) {
    extra.openai_model_normalization_enabled = false
  } else {
    delete extra.openai_model_normalization_enabled
  }
