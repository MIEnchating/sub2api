...
describe('EditAccountModal', () => {
  ...
  it('defaults OAuth model normalization on and persists an explicit opt-out', async () => {
    ...
    const wrapper = mountModal(account)
    const toggle = wrapper.get('[data-testid="edit-openai-model-normalization-toggle"]')
    expect(toggle.attributes('aria-checked')).toBe('true')
    ...
    expect(updateAccountMock).toHaveBeenCalledTimes(1)
    expect(updateAccountMock.mock.calls[0]?.[1]?.extra?.openai_model_normalization_enabled).toBe(
      false
    )
  })
