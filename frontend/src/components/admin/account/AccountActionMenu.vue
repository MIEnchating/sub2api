      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button @click="$emit('tool-diagnostics', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="search" size="sm" class="text-amber-500" />
              {{ t('admin.accounts.toolDiagnostics.open') }}
            </button>
