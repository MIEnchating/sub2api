export default {
  promptAudit: {
    ...
    pool: {
      ...
      secretHint: '明文只在本次编辑内存中存在；保存成功后会立即清除。', clearSecret: '显式清除已保存的 API Key', modelHint: 'Qwen3Guard 保持原生输出协议；其他模型自动使用带审计提示词的结构化 JSON 输出并消耗模型额度。', timeout: '总超时（毫秒）', inputLimit: '单片 Unicode 字符上限',
