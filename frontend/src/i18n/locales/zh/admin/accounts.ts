export default {
    accounts: {
      ...
      toolDiagnostics: {
        title: '工具诊断',
        open: '工具诊断',
        window: '最近 24 小时的 OpenAI 工具转发记录',
        time: '时间',
        status: '状态',
        inbound: '收到的工具',
        outbound: '转发的工具',
        missing: '缺失工具',
        requestId: '请求 ID',
        empty: '最近 24 小时没有工具诊断记录',
        loadFailed: '加载工具诊断失败',
        unknown: '待检测',
        diagnoses: {
          tools_present: '正常',
          no_tools_declared: '未声明工具',
          gateway_dropped_tools: '网关丢失工具',
          gateway_dropped_discovered_tools: '发现工具转发丢失',
          client_discovered_tools_not_declared: '工具搜索续链异常',
          tool_history_without_declarations: '工具历史缺少声明',
          tools_disabled: '工具已关闭',
          continuation_context_not_inspected: '续链上下文待确认',
          invalid_payload: '请求格式异常',
          unknown: '待检测'
        }
      },
      viewColumns: '列显示',
      ...
      openai: {
        ...
        legacyProtectionTitle: '旧版预设配置',
        legacyProtectionDescription: '此账号仍在使用旧版组合策略。若需独立调整它管理的指纹或连接设置，请先恢复启用前配置；自适应并发可单独设置。',
        legacyProtectionRestore: '恢复启用前配置',
        legacyProtectionRestoreTitle: '恢复启用前配置？',
        legacyProtectionRestoreMessage: '立即恢复旧策略管理的身份和连接设置，解除组合策略；之后手动调整过的并发值会保留。此操作不改变自适应并发设置。',
        legacyProtectionRestored: '已恢复启用前配置，可独立调整账号设置',
        legacyProtectionRestoreFailed: '恢复启用前配置失败',
        adaptiveConcurrencyTitle: '自适应并发控制',
        adaptiveConcurrencyDescription: '独立控制运行时并发，不修改 Codex 指纹和 TLS。启用后勾选“自适应并发”和“自动调整”，点击底部“更新”保存后生效。',
        adaptiveConcurrencyEnabled: '启用自适应并发控制',
        ...
        codexTicketAccountEnabledDesc: '使用网关配置的专用打票代理。Pro 使用 292，Team 使用 332；门票默认最多有效 240 秒，提前 60 秒续票。292 所需路由 Cookie 自动采集并在同账号模型间共享，不跨账号使用。',
