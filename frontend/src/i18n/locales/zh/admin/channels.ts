export default {
    ...
    riskControl: {
      ...
      refresh: '刷新',
      config: '内容审计配置',
      configHint: '使用 Moderation 模型或普通 GPT 模型进行请求内容评分，命中阈值后按模式处理。',
      openSettings: '内容审计设置',
      ...
      model: '模型名',
      modelHint: 'omni/text-moderation 模型使用 /v1/moderations；其他模型自动使用 /v1/chat/completions 结构化审计并消耗该模型额度。',
      ...
      hitAction: '风险命中动作',
      hitActionHint: '选择命中风险规则后的处理方式。路由只改变上游账号，用户计费和 API Key 权限仍按原分组处理。',
      hitActionBlock: '拦截请求',
      hitActionBlockDesc: '直接向客户端返回风控错误。',
      hitActionRouteGroup: '路由到分组',
      hitActionRouteGroupDesc: '在指定分组内按正常优先级和负载选择账号。',
      hitActionRouteAccount: '路由到账号',
      hitActionRouteAccountDesc: '强制使用指定账号，不参与普通账号池调度。',
      routeGroup: '风险请求目标分组',
      routeGroupPlaceholder: '选择目标分组',
      routeAccount: '风险请求目标账号',
      routeAccountPlaceholder: '搜索并选择目标账号',
      routeGroupRequired: '请选择风险请求的目标分组',
      routeAccountRequired: '请选择风险请求的目标账号',
      routeFailClosedHint: '目标不存在、不可调度、不支持请求模型或转发失败时，请求会被拒绝，不会退回原分组或普通账号池。WebSocket 已建立连接后的风险请求仍会直接拦截。',
