export default {
    ...
    riskControl: {
      ...
      configHint: 'Score request content with a Moderation or general GPT model and handle threshold hits by mode.',
      ...
      model: 'Model',
      modelHint: 'omni/text-moderation models use /v1/moderations; other models use structured /v1/chat/completions auditing and consume that model quota.',
      ...
      hitAction: 'Risk Hit Action',
      hitActionHint: 'Choose what happens after a risk rule matches. Routing changes only the upstream account; billing and API key authorization keep the original group.',
      hitActionBlock: 'Block request',
      hitActionBlockDesc: 'Return a risk-control error directly to the client.',
      hitActionRouteGroup: 'Route to group',
      hitActionRouteGroupDesc: 'Select an account from the target group using its normal priority and load rules.',
      hitActionRouteAccount: 'Route to account',
      hitActionRouteAccountDesc: 'Force the selected account and bypass the normal account pool.',
      routeGroup: 'Risk Request Target Group',
      routeGroupPlaceholder: 'Select a target group',
      routeAccount: 'Risk Request Target Account',
      routeAccountPlaceholder: 'Search and select a target account',
      routeGroupRequired: 'Select a target group for risk requests',
      routeAccountRequired: 'Select a target account for risk requests',
      routeFailClosedHint: 'If the target is missing, unavailable, incompatible with the requested model, or forwarding fails, the request is rejected without falling back to the original group or normal account pool. Risk hits after a WebSocket connection is established are still blocked.',
