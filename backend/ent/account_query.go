...
func (_q *AccountQuery) QueryAccountProxies() *AccountProxyQuery {
...
func (_q *AccountQuery) WithAccountProxies(opts ...func(*AccountProxyQuery)) *AccountQuery {
	query := (&AccountProxyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withAccountProxies; query != nil {
		if err := _q.loadAccountProxies(ctx, query, nodes,
			func(n *Account) { n.Edges.AccountProxies = []*AccountProxy{} },
			func(n *Account, e *AccountProxy) { n.Edges.AccountProxies = append(n.Edges.AccountProxies, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AccountQuery) loadAccountProxies(ctx context.Context, query *AccountProxyQuery, nodes []*Account, init func(*Account), assign func(*Account, *AccountProxy)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(accountproxy.FieldAccountID)
	}
	query.Where(predicate.AccountProxy(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.AccountProxiesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
