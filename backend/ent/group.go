...
type Group struct {
	...
	// Codex 额度透支分组覆盖；NULL 表示继承全局设置
	CodexQuotaOverdraftEnabled *bool `json:"codex_quota_overdraft_enabled,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldCodexQuotaOverdraftEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field codex_quota_overdraft_enabled", values[i])
			} else if value.Valid {
				_m.CodexQuotaOverdraftEnabled = new(bool)
				*_m.CodexQuotaOverdraftEnabled = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	if v := _m.CodexQuotaOverdraftEnabled; v != nil {
		builder.WriteString("codex_quota_overdraft_enabled=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
