...
INSERT INTO batch_image_jobs (
    batch_id, user_id, api_key_id, account_id, routed_group_id, provider, model, task_name, parent_batch_id, status,
...
RETURNING `+batchImageJobColumns,
		params.BatchID, params.UserID, params.APIKeyID, params.AccountID, params.RoutedGroupID, params.Provider, params.Model, params.TaskName, params.ParentBatchID, params.Status,
...
id, batch_id, user_id, api_key_id, account_id, routed_group_id, provider, model, task_name, parent_batch_id, status,
provider_job_name, provider_input_ref, provider_output_ref, gcs_input_uri, gcs_output_uri,
item_count, success_count, fail_count, cancelled_count,
estimated_cost, hold_amount, actual_cost,
base_unit_price, group_rate_multiplier, account_rate_multiplier,
...
func scanBatchImageJob(row rowScanner) (*service.BatchImageJob, error) {
	...
	var apiKeyID, accountID, routedGroupID sql.NullInt64
	...
	err := row.Scan(
		&job.ID, &job.BatchID, &job.UserID, &apiKeyID, &accountID, &routedGroupID, &job.Provider, &job.Model, &job.TaskName, &parentBatchID, &job.Status,
	...
	job.RoutedGroupID = batchImageNullInt64Ptr(routedGroupID)
