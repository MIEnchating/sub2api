...
func openCodeGoMergeMockColumns() []string {
	return []string{
		...
		"current_extra", "status", "schedulable", "error_message",
	}
}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
				...
				WillReturnRows(sqlmock.NewRows(openCodeGoMergeMockColumns()).
					AddRow(false, false, tt.proxyUnchanged, nil, nil, nil, nil, nil, nil, tt.groupUnchanged, tt.databaseAuto, tt.databaseSnapshot, nil, service.StatusActive, true, ""))
