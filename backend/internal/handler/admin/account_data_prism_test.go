...
func TestPrismExportDropsSessionAndDisablesRoutingWithoutMutatingSource(t *testing.T) {
	...
	prismExtra, ok := account.Extra[service.PrismExtraKey].(map[string]any)
	require.True(t, ok)
	require.Equal(t, false, prismExtra["enabled"])
	require.Equal(t, float64(240), prismExtra["timeout_seconds"])
