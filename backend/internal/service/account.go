...
// GetRateLimit429RetryCount returns the account's additional 429 retry budget.
...
// OpenAIModelNormalizationEnabledExtraKey is the account extra key controlling
// OpenAI OAuth/Codex model alias normalization before upstream forwarding.
const OpenAIModelNormalizationEnabledExtraKey = "openai_model_normalization_enabled"
...
// IsOpenAIModelNormalizationEnabled reports whether known Codex model aliases
// should be rewritten before forwarding. Existing accounts default to enabled;
// only an explicit false on an OpenAI account disables normalization.
func (a *Account) IsOpenAIModelNormalizationEnabled() bool {
	if a == nil || !a.IsOpenAI() || a.Extra == nil {
		return true
	}
	enabled, ok := a.Extra[OpenAIModelNormalizationEnabledExtraKey].(bool)
	return !ok || enabled
}
