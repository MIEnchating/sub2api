...
func OpenAICodexTicketStatuses(account *Account, cfg config.OpenAICodexTicketConfig, now time.Time) []OpenAICodexTicketStatus {
	...
	for _, model := range models {
		...
		cookies := parseOpenAICodexTicketCookies(account, openAICodexTicketScope(account, openAICodexTicketProxyRoute(account, cfg.HarvestProxyURL)))
		if cookies != nil {
...
func (s *OpenAIGatewayService) fireOpenAICodexTicketProbe(ctx context.Context, account *Account, token, model, proxyURL string, attemptTimeout time.Duration, routes ...codexTicketProxyRoute) (state string, status int, err error) {
	...
	if attemptCtx.Err() == nil {
		s.captureOpenAICodexTicketCookies(attemptCtx, account, resp.Header, time.Now(), routes...)
	}
...
func (s *OpenAIGatewayService) probeOnceOpenAICodexTicket(ctx context.Context, account *Account, model string) {
	...
	s.startOpenAICodexTicketProbe(ctx, account, model, s.openAICodexTicketHarvestConfig(ctx), time.Now(), false)
}
