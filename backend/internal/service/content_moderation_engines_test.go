...
func TestContentModerationEngineSwitchPreservesRiskRouting(t *testing.T) {
	repo := &contentModerationRuntimeSettingRepo{values: map[string]string{
		SettingKeyRiskControlEnabled: "true",
	}}
	s := runtimeCacheTestService(repo, time.Hour)
	accountID := int64(42)
	action := RiskHitActionRouteAccount
	engine := ContentModerationEngineTypeSafe
	view, err := s.UpdateConfig(context.Background(), UpdateContentModerationConfigInput{
		Engine: &engine, HitAction: &action, RouteAccountID: &accountID,
	})
	require.NoError(t, err)
	for _, selected := range []string{ContentModerationEngineTypeSafe, ContentModerationEngineOpenAI} {
		if selected != engine {
			view, err = s.UpdateConfig(context.Background(), UpdateContentModerationConfigInput{Engine: &selected})
			require.NoError(t, err)
		}
		require.Equal(t, action, view.HitAction)
		require.Equal(t, &accountID, view.RouteAccountID)
		for _, profile := range view.EngineConfigs {
			require.Equal(t, action, profile.HitAction)
			require.Equal(t, &accountID, profile.RouteAccountID)
		}
		target, ok := s.RiskHitRoutingTarget(context.Background())
		require.True(t, ok)
		require.Equal(t, RiskRoutingTarget{Action: action, AccountID: accountID}, target)
	}
}
