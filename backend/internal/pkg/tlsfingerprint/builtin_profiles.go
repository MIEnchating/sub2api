...
// BuiltinProfile returns a complete profile for the compatibility presets
// used by the account protection strategies. These are protocol templates;
// they do not claim to change upstream model quality.
func BuiltinProfile(name string) *Profile {
	...
	case "nodejs24":
		p = Profile{
			...
			ALPNProtocols:       []string{"http/1.1"}, SupportedVersions: []uint16{0x0304, 0x0303}, KeyShareGroups: []uint16{29}, PSKModes: []uint16{1},
	...
	case "nodejs22":
		p = Profile{
			...
			SupportedVersions: []uint16{0x0304, 0x0303}, KeyShareGroups: []uint16{29}, PSKModes: []uint16{1}, Extensions: []uint16{0, 11, 10, 35, 16, 22, 23, 13, 43, 45, 51},
		}
	default:
		return nil
	}
