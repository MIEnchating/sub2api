-- The custom per-user concurrency limit is still used by the group repository.
-- Restore its schema on fresh installs after the legacy migration was retired;
-- preserve configured limits on existing databases.
ALTER TABLE groups
