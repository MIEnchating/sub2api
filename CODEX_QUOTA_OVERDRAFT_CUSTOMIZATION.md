...
- 当前官方合并基线：Sub2API `v0.1.179`，`upstream/main` 提交 `4033387fd`
...
- 修复上游 Codex 用量快照短暂回落到 99% 时错误结束透支周期的问题。
- 透支周期继续使用已持久化的窗口起点和冻结恢复时间，避免后续快照漂移重算统计边界。
- 透支期统计不再依赖当前利用率必须保持 100%，仍按 `usage_logs` 汇总请求数、Token 和账号费用。
- 前端保留小额透支费用的有效精度，避免真实费用显示为 `$0.00`。
